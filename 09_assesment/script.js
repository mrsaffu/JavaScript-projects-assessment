...
// import { displaySearchedProduct } from "./searchedProduct.js";
...
let totalValue = totalPrice(product);
let total = (document.getElementById("total").innerText = totalValue);
...
searchButton.addEventListener("click", function () {
  ...
  let searchedProdect = product.filter(({ name }) => {
    name = name.toLowerCase();
    return name.includes(searchValue);
  });

  console.log(searchedProdect);

  if (searchedProdect[0] === undefined) {
    console.log("Hi");
    table.innerHTML += `<h3 >product is not available...!</h3>
        `;
  } else {
    displayProduct(searchedProdect);

    let totalValue = totalPrice(searchedProdect);

    table.innerHTML += `
    <tfoot>
      <tr>
         <th colspan="4">Total</th>
        <th id="total">${totalValue}</th>
      </tr>
    </tfoot> `;
  }
});
